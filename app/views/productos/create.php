...
document.addEventListener('DOMContentLoaded', function() {
    // Ya no requerimos lógica local de UI para fecha de vencimiento.
    ...
    function showPreview(file) {
        ...
        reader.onload = (e) => {
            preview.innerHTML = `
                ...
                <small class="text-muted mt-2">${file.name} (${(file.size/1024).toFixed(0)} KB)</small>
            `;
        };
    ...
    if(perecederoSwitch) {
        perecederoSwitch.addEventListener('change', function() {
            if (this.checked) {
                perecederoAlert.classList.remove('d-none');
            } else {
                perecederoAlert.classList.add('d-none');
            }
        });
    }
