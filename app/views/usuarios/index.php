    ...
    <div class="form-group">
        ...
        <select name="rol" class="form-select">
            ...
            <?php foreach ($roles as $rol): ?>
                <option value="<?= $rol->id ?>" <?= $rolFilter == $rol->id ? 'selected' : '' ?>>
                    <?= htmlspecialchars($rol->nombre) ?></option>
            <?php endforeach; ?>
        </select>
    </div>
    <div class="form-group">
    ...
    <div class="card-body p-0">
        <?php if (empty($usuarios['data'])): ?>
            <div class="empty-state" style="padding: 3.5rem 1rem;">
                ...
                <p class="text-muted mb-3" style="max-width:320px;">No encontramos usuarios con esos criterios. Intente con
                    otro filtro o agregue uno nuevo.</p>
                <?php if (hasPermission('usuarios.crear')): ?>
                    ...
                    <tbody>
                        <?php foreach ($usuarios['data'] as $u): ?>
                            <tr class="<?= !$u->activo ? 'row-inactive' : '' ?>">
                                ...
                                <td><span
                                        class="badge <?= roleBadgeClass($u->rol_nombre ?? '') ?>"><?= $u->rol_nombre ?? '-' ?></span>
                                </td>
                                <td>
                                    <?php if ($u->activo): ?>
                                ...
                                <td><small
                                        class="text-muted"><?= $u->ultimo_login ? formatDate($u->ultimo_login) : 'Nunca' ?></small>
                                </td>
                                <td>
                                    <div class="d-flex gap-1">
                                        <a href="<?= url("usuarios/ver/{$u->id}") ?>" class="btn-action btn-view"
                                            title="Ver detalle">
                                            <i class="bi bi-eye-fill"></i>
                                        </a>
                                        <?php if (hasPermission('usuarios.editar')): ?>
                                            <a href="<?= url("usuarios/editar/{$u->id}") ?>" class="btn-action btn-edit"
                                                title="Editar">
