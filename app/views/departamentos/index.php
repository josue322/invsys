                    ...
                    <?php else: ?>
                        <?php foreach ($departamentos as $d): ?>
                            <tr>
                                ...
                                <td><span
                                        class="badge bg-light text-dark border"><?= htmlspecialchars($d->centro_costo ?: 'N/A') ?></span>
                                </td>
                                ...
                                <?php if (hasPermission('departamentos.crear') || hasPermission('departamentos.eliminar')): ?>
                                    <td class="text-end pe-4">
                                        <div class="d-flex justify-content-end gap-1">
                                            <?php if (hasPermission('departamentos.crear')): ?>
                                                <a href="<?= url('departamentos/edit/' . $d->id) ?>" class="btn-action btn-edit"
                                                    title="Editar">
                                                    <i class="bi bi-pencil-fill"></i>
                                                </a>

                                                <!-- Toggle activa/inactiva -->
                                                <form method="POST" action="<?= url("departamentos/toggle/{$d->id}") ?>"
                                                    ...
                                                    <?= csrfField() ?>
                                                    <button type="submit"
                                                        class="btn-action <?= $d->activo ? 'btn-toggle-off' : 'btn-toggle-on' ?>"
                                            ...
                                            <?php if (hasPermission('departamentos.eliminar')): ?>
                                                <form method="POST" action="<?= url("departamentos/eliminar/{$d->id}") ?>"
                                                    style="display:inline" data-confirm='<?= json_encode([
        ...
        <nav aria-label="Page navigation">
            <ul class="pagination pagination-sm">
                <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                    <a class="page-link"
                        href="<?= url('departamentos?page=' . ($page - 1) . '&search=' . urlencode($search)) ?>">Anterior</a>
                </li>
                <?php for ($i = 1; $i <= $last_page; $i++): ?>
                    <li class="page-item <?= $page == $i ? 'active' : '' ?>">
                        <a class="page-link"
                            href="<?= url('departamentos?page=' . $i . '&search=' . urlencode($search)) ?>"><?= $i ?></a>
                    </li>
                <?php endfor; ?>
                <li class="page-item <?= $page >= $last_page ? 'disabled' : '' ?>">
                    <a class="page-link"
                        href="<?= url('departamentos?page=' . ($page + 1) . '&search=' . urlencode($search)) ?>">Siguiente</a>
                </li>
            </ul>
        </nav>
    </div>
